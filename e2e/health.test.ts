...
describe('Health Records E2E', () => {
  ...
  describe('Navigation to Health Page', () => {
    beforeAll(async () => {
      page = await miniProgram.reLaunch('/pages/index/index');
      await page.waitFor(500);
    });

    it('should navigate to health page when tapping menu item', async () => {
      const menuItem = await page.$('.menu-item:not(.disabled)');
      expect(menuItem).not.toBeNull();
      ...
      expect(page.path).toBe('pages/health/index/index');
    });

    it('should display health page title', async () => {
      const title = await page.$('.title');
      expect(title).not.toBeNull();
      const text = await title!.text();
  ...
  describe('Add Health Record', () => {
    beforeAll(async () => {
      page = await miniProgram.reLaunch('/pages/health/add/index');
      await page.waitFor(500);
    });

    it('should load add page correctly', async () => {
      expect(page.path).toBe('pages/health/add/index');
    });
  ...
  describe('Health Records List', () => {
    beforeAll(async () => {
      page = await miniProgram.reLaunch('/pages/health/index/index');
      await page.waitFor(1000);
    });
